...
export default function CreateAccountChatPage({ searchParams }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const msgs = res.data.messages || res.data.Allmessages || [];
      
      // Map messages to match the format expected by the UI
      ...
      let idx = formattedMessages.filter((m) => m.role === "user").length;
      setStepIndex(idx);
